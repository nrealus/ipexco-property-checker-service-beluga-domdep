...
export function create_property_check_run(
	...
	return {
		...
		exp_path: `${process.env.TEMP_RUN_FOLDERS}/${request.id}`,
	};
}
...
export async function schedule_run(run: PropertyCheckRun, job: Job<any>) {
	...
	cleanUpRunEnvironment(run.exp_path);
}
...
function runProcess(
	...
	return new Promise((resolve, reject) => {
		...
		try {
			const checkProcess: ChildProcess = spawn(process.env.PROPERTY_CHECKER, args, options);
